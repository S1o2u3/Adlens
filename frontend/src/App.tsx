...
function generateInsights(campaigns: Campaign[]): Insight[] {
  ...
  insights.push({
    ...
    message: "Search avg ROAS: 3.5x | Display avg ROAS: 2.8x | Video avg ROAS: 4.0x. Compare your campaigns against these thresholds.",
  });
  return insights;
}
...
You are a senior Google Ads analyst at Google's gTech Ads team with 10 years of experience.

INDUSTRY BENCHMARKS:
- Search: avg CTR 3.17%, avg CPC $2.69, avg ROAS 3.5x
- Display: avg CTR 0.46%, avg CPC $0.63, avg ROAS 2.8x
- Video: avg CTR 0.514%, avg ROAS 4.0x

ROAS THRESHOLDS:
- Below 2x: Losing money — pause immediately
- 2x-3x: Marginal — optimize before scaling
- 3x-5x: Healthy — maintain and test increases
- Above 5x: Strong — increase budget 20-30%

INDUSTRY BENCHMARKS BY VERTICAL:
- Retail: avg ROAS 4.0x | Tech: avg ROAS 3.8x | Finance: avg ROAS 3.2x
- SaaS: avg ROAS 4.2x | Healthcare: avg ROAS 3.0x | Gaming: avg ROAS 7.0x
- Education: avg ROAS 4.5x | Travel: avg ROAS 3.5x | Fashion: avg ROAS 3.8x

OPTIMIZATION PLAYBOOK:
- Low CTR on Search (<2%): Rewrite ad copy, add RSA variations, review keyword match types
- High CPC: Switch to Target CPA bidding, add negative keywords, improve Quality Score
- Low ROAS on Display: Tighten audience segments, exclude low-intent placements
- Low CTR on Display (<0.4%): Creative fatigue — rotate new banner sizes
- Strong Video ROAS: Increase budget, test unskippable formats, expand similar audiences
- Paused campaigns with good historical ROAS: Reactivate with refined targeting

BUDGET ALLOCATION RULES:
- Top performers (ROAS >5x): Allocate 40% of total budget
- Mid performers (ROAS 3-5x): Allocate 40% of total budget
- Underperformers (ROAS <3x): Max 20% budget, focus on optimization

SEASONAL PATTERNS:
- Q2 (Apr-Jun): Steady growth, ideal for testing new campaigns
- Q4 (Oct-Dec): 35% higher CPCs, 60% higher conversion rates
- Summer: Travel, food, gaming perform above average

Give specific data-driven recommendations with exact numbers. Reference industry benchmarks. Be direct and actionable like a real analyst. Always mention specific campaign names from the data.
`;
...
export default function App() {
  ...
  function exportCSV() {
    const headers = ["Name", "Type", "Status", "Budget", "Spend", "CTR", "CPC", "ROAS", "Industry", "Region"];
    const rows = campaigns.map(c => [c.name, c.type, c.status, c.budget, c.spend, c.ctr, c.cpc, c.roas, c.industry ?? "", c.region ?? ""]);
    const csv = [headers, ...rows].map(r => r.join(",")).join("\n");
    const blob = new Blob([csv], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = "adlens_campaigns.csv";
    a.click();
  }
...
LIVE CAMPAIGN DATA (${campaigns.length} campaigns across ${[...new Set(campaigns.map(c => c.industry))].length} industries):
    ...
    <div style={{ fontFamily: "sans-serif", background: "#f8f9fa", minHeight: "100vh" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "1.25rem 2rem", background: "white", borderBottom: "0.5px solid #e8eaed" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <span style={{ fontSize: 11, color: "#5f6368" }}>{campaigns.length} campaigns · {[...new Set(campaigns.map(c => c.industry))].length} industries</span>
      ...
      <div style={{ padding: "1.5rem 2rem" }}>
        ...
        {stats && (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12, marginBottom: "1.5rem" }}>
            {[
              ...
              { label: "Avg ROAS", value: `${stats.avgROAS}x`, color: "#BA7517", delta: stats.avgROAS >= 3.5 ? "+vs industry 3.5x" : "-vs industry 3.5x", up: stats.avgROAS >= 3.5 },
              { label: "Conversions", value: stats.totalConversions.toLocaleString(), color: "#E24B4A", delta: "+23% vs last period", up: true },
            ].map((card) => (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 320px", gap: 16, marginBottom: "1.5rem" }}>
          ...
          <div style={{ background: "white", borderRadius: 12, border: "0.5px solid #e8eaed", padding: "1.25rem" }}>
            <div style={{ fontSize: 13, fontWeight: 500, marginBottom: 4 }}>Spend vs ROAS by campaign</div>
            <div style={{ fontSize: 11, color: "#5f6368", marginBottom: 12 }}>Top 10 campaigns · Industry ROAS benchmark: 3.5x</div>
            <ResponsiveContainer width="100%" height={220}>
              <ComposedChart data={chartData} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#f1f3f4" />
                <XAxis dataKey="name" tick={{ fontSize: 9, fill: "#5f6368" }} />
                ...
                <Line yAxisId="right" type="monotone" dataKey="roas" stroke="#639922" strokeWidth={2} dot={{ fill: "#639922", r: 3 }} />
              </ComposedChart>
            </ResponsiveContainer>
          ...
          <div style={{ background: "white", borderRadius: 12, border: "0.5px solid #e8eaed", padding: "1.25rem", display: "flex", flexDirection: "column", gap: 10 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
              ...
              <span style={{ fontSize: 10, color: "#5f6368", marginLeft: "auto" }}>Powered by Groq</span>
            </div>
            {insights.map((ins, i) => {
            ...
            {aiResponse && (
              <div style={{ background: "#E6F1FB", borderRadius: 8, padding: "10px 12px", fontSize: 12, lineHeight: 1.5, color: "#0C447C", maxHeight: 200, overflowY: "auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: "1.5rem" }}>
          ...
          {/* Pie Chart */}
          <div style={{ background: "white", borderRadius: 12, border: "0.5px solid #e8eaed", padding: "1.25rem" }}>
            <div style={{ fontSize: 13, fontWeight: 500, marginBottom: 4 }}>Spend by campaign type</div>
            <div style={{ fontSize: 11, color: "#5f6368", marginBottom: 12 }}>Across all {campaigns.length} campaigns</div>
          ...
          <div style={{ background: "white", borderRadius: 12, border: "0.5px solid #e8eaed", padding: "1.25rem" }}>
            ...
            <button onClick={() => askAI("Give me a full performance report with specific campaign names, compare against industry benchmarks, and give top 3 prioritized action items with exact budget recommendations")}
              style={{ width: "100%", padding: "10px 16px", borderRadius: 8, border: "0.5px solid #e8eaed", background: "#f8f9fa", color: "#378ADD", fontSize: 13, cursor: "pointer", marginBottom: 10 }}>
              Generate AI performance report
            </button>
            <button onClick={() => askAI("Which campaigns should I increase budget on and by how much? Which should I pause? Give specific dollar amounts based on current spend and ROAS")}
            ...
            {stats && (
              <div style={{ marginTop: 16, padding: 12, background: "#f8f9fa", borderRadius: 8, fontSize: 12, color: "#5f6368", lineHeight: 1.8 }}>
                <div>Total campaigns: <strong>{campaigns.length}</strong></div>
        ...
        <div style={{ background: "white", borderRadius: 12, border: "0.5px solid #e8eaed", overflow: "hidden" }}>
          <div style={{ padding: "1rem 1.25rem", borderBottom: "0.5px solid #e8eaed", fontSize: 13, fontWeight: 500, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span>All campaigns</span>
            <span style={{ fontSize: 11, color: "#5f6368" }}>{campaigns.length} total</span>
          </div>
              ...
              <tbody>
                {campaigns.map((c) => (
                  <tr key={c.id} style={{ borderBottom: "0.5px solid #f1f3f4" }}>
                    ...
                    <td style={{ padding: "10px 14px", color: "#5f6368", fontSize: 11 }}>{c.industry ?? "—"}</td>
                    <td style={{ padding: "10px 14px" }}>
